...
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> { //turns on the logging system that prints what the server is doing
    ...
    // load settings from env varibkes, connect ot db, create app state
    ...
    let protected = Router::new()
        ...
        .layer(middleware::from_fn_with_state(state.clone(), require_login)); // "security guaard" for protected routes

    //create app and merge public and protected routes
    let app = Router::new()
        .merge(public)
        .merge(protected)
